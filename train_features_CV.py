...
test_data = conv_to_number(test_data)
#test_data = test_data.iloc[:10,:]
test_data_loader_all = create_data_loader(test_data, tokenizer, MAX_LEN, BATCH_SIZE)
...
def eval_model(model, data_loader, loss_fn, device, n_examples):
  ...
  with torch.no_grad():
    for d in data_loader:
      ...
      outputs = model(
        input_ids=input_ids,
        attention_mask=attention_mask,d1 = d1,d2=d2,pos1=pos1,pos2=pos2,LSTM_output=LSTM_output,device=device
      )
      _, preds = torch.max(outputs, dim=1)
      loss = loss_fn(outputs, labels)
      correct_predictions += torch.sum(preds == labels)
...
# ==============================================
    for epoch in range(EPOCHS):
          ...
          print("\n")
          print("Runing Testing on testing set")
            
          test_acc_all, test_loss_all = eval_model(
               model,
          ...
          print(f'Test-ALL   loss {test_loss_all} accuracy {test_acc_all}')
          
          elapsed = format_time(time.time() - t0)
          print(f'Testing took {elapsed}')
          print()
          train_acc_.append(train_acc.item())
          train_loss_.append(train_loss.item())
          test_acc_.append(test_acc.item())
          test_loss_.append(test_loss.item())
          
          test_acc_all_.append(test_acc_all.item())
          test_loss_all_.append(test_loss_all.item())
          if test_acc_all > best_accuracy:
            best_accuracy = test_acc


            torch.save(model.state_dict(), 'best_model_state_lstm'+str(BATCH_SIZE)+'_'+str(EPOCHS)+'.bin')
            best_accuracy = test_acc
          ...
          #  break
    print('Accuracy for fold ', (fold,test_acc.item()))
    print('Loss for fold ', (fold,test_loss))
    
###################################################################
    fold_acc.append(test_acc.item())
...
# ==============================================
    ...
    history.to_csv('progress_CV_'+str(BATCH_SIZE)+'_'+str(EPOCHS)+'.csv')
    
    EVAL_TEST_SET = pd.DataFrame()
    EVAL_TEST_SET["test_acc"]=test_acc_all_
    EVAL_TEST_SET["test_loss"] = test_loss_all_
    EVAL_TEST_SET.to_csv('progress_all_testset_CV_lstm_'+str(BATCH_SIZE)+'_'+str(EPOCHS)+'.csv')
    folds_res = pd.DataFrame()
    folds_res["acc"] = fold_acc
    folds_res["loss"] = fold_loss
    folds_res.to_csv("folds_res_CV_lstm_"+str(BATCH_SIZE)+"_"+str(EPOCHS)+".csv")
#######################
print("")
...
results_Bert_sci["y_pred"] = [pred.item() for pred in  y_pred]
results_Bert_sci["y_pred_probs"] = y_pred_probs
results_Bert_sci["real_values"]  = [real.item() for real in y_test]
results_Bert_sci.to_csv("predictions_Bert_sci_CV_lstm_"+str(BATCH_SIZE)+"_"+str(EPOCHS)+".csv")
